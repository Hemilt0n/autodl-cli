...
- 单实例命令支持 `-n/--name`：通过当前账号实例列表按名称解析 UUID，再执行 `status`、`inspect`、`start`、`stop`、`release`、`destroy`。
...
- 名称解析只做精确匹配；名称缺失、未匹配或重复匹配时只提示用户改用 UUID，不继续执行计费或破坏性操作。
...
1. 默认使用 `auto`：优先写入系统 keyring。
2. 如果 keyring 不可用，自动降级写入用户数据目录下的 token 文件。
3. 写入 token 文件时，权限设置为当前用户可读写。
4. 命令输出永远不打印完整 token。
...
2. Token 存储是否接受默认使用 auto，即 keyring 不可用时写入本地 token 文件？
